...
- Why: `scripts/check-doc-drift.cjs` (the CI-enforced doc-accuracy gate) only validates 9 *numeric* claims (test/suite/coverage/agent/command/skill/hook counts) via regex — it has no mechanism to verify that a file path mentioned in prose actually exists. `scripts/validate-doc-links.cjs` only parses `[text](path)` Markdown link syntax, not backtick code-spans inside a fenced tree diagram. Both CI-blocking validators structurally cannot catch this class of drift.
