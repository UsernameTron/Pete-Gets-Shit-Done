...
8. **Failure & rollback:** audit `gaps_found` → GATE 1; anything failing before GATE 2 leaves the milestone open and resumable — nothing irreversible has happened yet. **Deliberately excluded:** `/gsd:finalize` — it contains two ungated `git push` steps (`finalize.md:65,143`), a tool-permission mismatch (AskUserQuestion absent from `allowed-tools` while its inlined complete-milestone needs it), and a cross-plugin spawn of `repo-doc-architect` that dangles if `claude-mcp-ecosystem` isn't enabled (`finalize.md:118`). Do not put `finalize` in an unattended chain until those are fixed.
