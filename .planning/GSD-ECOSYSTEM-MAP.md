...
Generated: 2026-07-11 | Repo: `Pete-Gets-Shit-Done` @ `80e2006` (main) | Engine: `get-shit-done-cc v1.30.0` | Baseline: GSD v2.7

Provenance: generated by live filesystem scan + baseline reconciliation (deterministic inventory script + 4 enrichment agents + cold-context second-model review). Every path below was verified to exist this session; nothing is from memory.

## Executive summary

- **235 components** discovered across 7 types: 79 commands, 61 workflows, 35 agents, 47 skills, 9 hooks, 3 plugins, 1 marketplace — every one placed in exactly one of 11 lifecycle clusters (8 archived, kept in their functional clusters).
- The five spine clusters C1–C5 hold 92 components; heaviest cluster is C10 (42).
- Biggest baseline drift: **GSD workflows 56 → 61 (Δ +5)** — the repo has moved past the v2.7 baseline; 3 baseline rows are workstation-scoped and unverifiable in this container.
- C-UNMAPPED: 2 item(s): join-discord, get-shit-done-cc (see Coverage gaps).
- Most useful takeaway: the operational surface is command-driven (75 user-facing commands routing to 61 workflows), but runtime *enforcement* (hooks) is workstation-bound — the repo itself registers only 2 live hooks.
...
Source: `plugins/*/.claude-plugin/plugin.json` + `package.json` + `plugins/.claude-plugin/marketplace.json`
...
Live system vs the embedded GSD v2.7 baseline. The live system wins; workstation-scoped rows are reported honestly as unverifiable in this container rather than guessed.

| Component type | Baseline | Discovered | Δ | Reason |
|---|---|---|---|---|
| Slash commands | 72 | 75 | +3 | Repo is post-v2.7 (package v1.30.0, docs reference v2.8 work): net +3 /gsd: core commands shipped since the baseline. The 4 engine-internal copies under get-shit-done/commands/gsd are counted separately (one, workstream… |
| GSD workflows | 56 | 61 | +5 | +5 workflows shipped since the v2.7 baseline (live get-shit-done/workflows/ = 61). |
| Built-in agents | 17 | 17 | 0 | Match. |
| Project-scoped agents | 3 | 3 | 0 | Match (plugin-developer, test-runner, docs-sync). |
| Archived agents | 8 | 8 | 0 | Match. |
| Factory subagents | 10 | 1 | -9 | UNVERIFIED here — generated subagents live on the workstation (~/.claude), which this container does not have. The repo ships 1 factory agent source (extension-builder). |
| Plugin skills | 45 | 45 | 0 | Match: 38 Factory + 7 MCP-Ecosystem. Two additional repo-local .claude/skills entries (dream-memory-consolidation + a duplicate loose SKILL.md) are counted separately. |
| Runtime hooks | 17 | UNVERIFIED (workstation scope) | — | UNVERIFIED here — the 17 registrations live in workstation ~/.claude/settings.json (NOT FOUND in this container). Repo-side surface: 6 hook sources + 2 live repo registrations + 1 unwired local hook. |
| Core plugins | 3 | 3 | 0 | Match (get-shit-done engine, claude-code-factory 1.0.0, claude-mcp-ecosystem 2.0.0). |
| Other registered plugins | 10 | UNVERIFIED (workstation scope) | — | UNVERIFIED here — registered in workstation ~/.claude/settings.json, NOT FOUND in this container. |
...
|---|---|---|---|---|---|
| `CLAUDE.md` | 14 | **66 slash commands** spanning discuss, plan, execute, verify, ship, milestone … | commands | 66 | 75 |
...
| `CLAUDE.md` | 18 | **5-phase delivery lifecycle**: discuss → plan → execute → verify → ship, with … | workflows | 5 | 61 |
...
| `CLAUDE.md` | 51 | **Scale**: 566 test suites, 2,861 assertions, 91.82% line coverage | tests | 566 | — (not measured this run) |
| `CLAUDE.md` | 51 | **Scale**: 566 test suites, 2,861 assertions, 91.82% line coverage | tests | 2861 | — (not measured this run) |
| `CLAUDE.md` | 51 | **Scale**: 566 test suites, 2,861 assertions, 91.82% line coverage | coverage | 91.82 | — (not measured this run) |
...
| `README.md` | 59 | 66 commands, 17 agents, 6 hooks, wave-based parallel execution | commands | 66 | 75 |
| `README.md` | 59 | 66 commands, 17 agents, 6 hooks, wave-based parallel execution | agents | 17 | 17 |
| `README.md` | 59 | 66 commands, 17 agents, 6 hooks, wave-based parallel execution | hooks | 6 | 6 |
...
| `README.md` | 225 | Before we get into the 64 commands that make this thing work, the syntax varies… | commands | 64 | 75 |
| `README.md` | 1110 | **Safety hooks** — 15 hooks fire automatically during Claude Code's lifecycle e… | hooks | 15 | 6 |
| `README.md` | 1186 | **Session commands table** — 30 commands across GSD, MCP ecosystem, and utiliti… | commands | 30 | 75 |
...
| `docs/DEVOPS-HANDOFF.md` | 46 | Commands \| `~/.claude/get-shit-done/commands/` \| 66 GSD slash commands | commands | 66 | 75 |
...
Live basis: 75 user-facing commands (66 `/gsd:` + 9 eco), 61 workflows, 17 built-in agents, 45 plugin skills, 6 hook sources. Precedent: `scripts/check-doc-drift.cjs` already reconciles a subset of these in CI.
...
| gsd-statusline | hook | Claude Code Statusline - GSD Edition Shows: model \| current task \| directory \| co… | — |
...
**28 components** — 12 commands, 3 hooks, 4 skills, 9 workflows
...
**20 components** — 8 agents, 5 commands, 7 workflows (of which 2 archived)
...
| /gsd:validate-phase | command | Retroactively audit and fill Nyquist validation gaps for a completed phase | — |
...
| /gsd:crew | command | Agent roster, capability map, and self-assessment — show your team and find gaps | — |
...
| /gsd:thread | command | Manage persistent context threads for cross-session work | — |
...
**29 components** — 3 agents, 12 commands, 2 hooks, 12 workflows
...
| Component | Type | Primary role in this stage | Secondary stages |
|---|---|---|---|
| extension-validator *(archived)* | agent | Read-only quality gate that validates Claude Code extensions against official docume… | — |
...
| /gsd:new-project | command | Initialize a new project with deep context gathering and PROJECT.md | — |
...
| new-project | workflow | Initialize a new project through unified flow: questioning, research (optional), req… | — |
...
**27 components** — 6 agents, 10 commands, 4 hooks, 7 workflows (of which 1 archived)
...
**235 rows** — equals the component total; every inventory item appears exactly once.
...
2. **Docs disagree with the filesystem and each other**  — README states 66, 64, and 30 commands and 6, 10, and 15 hooks in different sections; CLAUDE.md describes lib/*.cjs modules that are not in lib/ (it holds 2 JSON pattern files; engine logic lives in get-shit-done/bin). Fix: extend scripts/check-doc-drift.cjs to these surfaces.
...
*Append one row per regeneration; the proposed /gsd:ecosystem-map command automates this.*
