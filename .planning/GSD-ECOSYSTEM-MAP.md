...
Generated: 2026-07-11 | Repo: `Pete-Gets-Shit-Done` @ `af2adba` (main) | Engine: `get-shit-done-cc v1.30.0` | Baseline: GSD v2.7
...
- **237 components** discovered across 7 types: 80 commands, 62 workflows, 35 agents, 47 skills, 9 hooks, 3 plugins, 1 marketplace — every one placed in exactly one of 11 lifecycle clusters (8 archived, kept in their functional clusters).
- The five spine clusters C1–C5 hold 92 components; heaviest cluster is C10 (42).
- Biggest baseline drift: **GSD workflows 56 → 62 (Δ +6)** — the repo has moved past the v2.7 baseline; 3 baseline rows are workstation-scoped and unverifiable in this container.
- C-UNMAPPED: 2 item(s): join-discord, get-shit-done-cc (see Coverage gaps).
- Most useful takeaway: the operational surface is command-driven (76 user-facing commands routing to 62 workflows), but runtime *enforcement* (hooks) is workstation-bound — the repo itself registers only 2 live hooks.
...
|---|---|---|---|---|
| Slash commands | 72 | 76 | +4 | Repo is post-v2.7 (package v1.30.0, docs reference v2.8 work): net +4 /gsd: core commands since the baseline (67, including /gsd:ecosystem-map added by this run). The 4 engine-internal copies under get-shit-done/command… |
| GSD workflows | 56 | 62 | +6 | +6 workflows since the v2.7 baseline (live get-shit-done/workflows/ = 62, including ecosystem-map.md added by this run). |
...
|---|---|---|---|---|---|
| `CLAUDE.md` | 14 | **66 slash commands** spanning discuss, plan, execute, verify, ship, milestone … | commands | 66 | 76 |
...
| `CLAUDE.md` | 18 | **5-phase delivery lifecycle**: discuss → plan → execute → verify → ship, with … | workflows | 5 | 62 |
...
| `README.md` | 59 | 66 commands, 17 agents, 6 hooks, wave-based parallel execution | commands | 66 | 76 |
...
| `README.md` | 225 | Before we get into the 64 commands that make this thing work, the syntax varies… | commands | 64 | 76 |
...
| `docs/DEVOPS-HANDOFF.md` | 46 | Commands \| `~/.claude/get-shit-done/commands/` \| 66 GSD slash commands | commands | 66 | 76 |
...
**237 rows** — equals the component total; every inventory item appears exactly once.
