...
### Added
- **`ship-milestone` workflow (W5, unshelved)** — `get-shit-done/workflows/ship-milestone.md`: one intent ("close out the milestone") drives health → audit-agents `--no-commit` → sync-docs → coverage+drift → audit-milestone → conditional GATE 1 (fires only when the audit is not `passed`) → ship/ci-watch → GATE 2 → complete-milestone, whose 3 internal prompts (archive phases, branch handling, tag push) stay live human prompts — GATE 2 authorizes starting the sequence, never answers for it. Registry-routable as `workflow:ship-milestone` (13th named flow); blind routing spot-check 3/3 confirmed description distinctness vs `ship-and-merge` and the bare `complete-milestone` primitive. Contract test `tests/ship-milestone.test.cjs`; golden intent id 26 added to the permanent routing fixture. Shelf lifted after the finalize push-consent repairs and Phase 58's Gate 5.5 hardening + end-to-end re-verification (Phase 59)
- **`gsd-spawn-tracker` PreToolUse hook** — records every subagent spawn (Task/Agent tool) to `~/.claude/metrics/spawns.jsonl` as `{timestamp, session_id, tool, subagent_type, description}`, giving GSD first-class per-agent spawn attribution. The SubagentStop health hook cannot identify which agent completed, so spawn frequency was previously unmeasurable except by inference. Non-blocking (always exits 0), mirrors the `gsd-cost-tracker` pattern; bundled into `hooks/dist/` and registered as a PreToolUse `Task|Agent` hook by both installers. Contract test `tests/spawn-tracker-hook.test.cjs` (16 cases)
- **`smart-discuss --auto` grey-area fold** — `get-shit-done/workflows/smart-discuss.md` gains an `AUTO_MODE` path (from `--auto`, `workflow._auto_chain_active`, or `mode: yolo`): grey areas whose recommended answers are all groundable are accepted without `AskUserQuestion` and summarized in an `[auto]` receipt table; only unresolvable areas still ask. `idea-to-shipped` calls it with `--auto` unconditionally (its two-gate contract holds via GATE 1 plan review); `autonomous.md` appends `--auto` only under yolo / an active chain. Interactive runs are byte-identical. Applies audit blueprint `BP-smart-discuss-auto-parity`
- **Review-pending Stop sentinel + lesson-capture gate registration** — the governance Stop-hook template now approves a dirty tree with a logged "Deferred review" note when `.planning/.review-pending` is present (and still blocks with exit 2 without it), so an explicit "let me review first" is no longer force-committed; the shipped-but-inert `.claude/hooks/lesson-capture-gate.cjs` is registered as a project Stop hook. New reference `get-shit-done/references/review-pending-sentinel.md`. Applies audit blueprint `bp-f-review-sentinel-lesson-gate`
- **`/gsd:finalize` push consent** — both pushes (Gate 1 unpushed work, Gate 7 finalize commit) are now consent-gated: `AskUserQuestion` interactively, or pre-approved by the new `--yes-push` flag or the `workflow.finalize_auto_push` config key (default `false`) with an `[auto-push]` receipt line; pre-approval never bypasses the Gate 2 build-health check. Unblocks routing `ship-milestone` through `finalize`. Applies audit blueprint `bp-a-finalize-push-consent`
- **`/gsd:do` workflow routing** — the intent router now dispatches three named autonomous flows via `workflow:` targets: session-start phrasing → `daily-startup`, "idea to PR" end-to-end phrasing → `idea-to-shipped`, wrap-up phrasing → `wrap-and-sync`; first-match ordering verified by `tests/do-routing.test.cjs`. One front door + named flows, per the entry-point recommendation in `.planning/GSD-AUTONOMOUS-WORKFLOWS.md`
- **`idea-to-shipped` workflow** — `get-shit-done/workflows/idea-to-shipped.md`: freeform idea → smart-discuss → plan → GATE 1 (approve plan) → execute → verify (default mode, never `--mode=schema`) → clean-context `gsd-verifier` review → GATE 2 (approve ship) → `ship --draft` → ci-watch. Plan-only early exit, 1-retry gap-closure and CI-red caps, `pause-work` failure handoff; merge stays human on GitHub. Flagship build from the autonomous-workflows design (W2). Contract test `tests/idea-to-shipped.test.cjs`
- **`smart-discuss` workflow** — `get-shit-done/workflows/smart-discuss.md`: the autonomous-mode batch-table discuss variant, extracted content-faithfully from `workflows/autonomous.md` (its CTRL-03 note anticipated this) so it is callable by both `/gsd:autonomous` and `idea-to-shipped`. `autonomous.md`'s `smart_discuss` step is now a thin caller — behavior-neutral, same CONTEXT.md output
- **`daily-startup` workflow** — `get-shit-done/workflows/daily-startup.md`: one-intent session start (prime-patterns boot → `/gsd:daily` dashboard → conditional checkpoint/handoff context restore with the interactive menu skipped → single recommended next command). Read-only end to end, zero gates — the only write is prime-patterns' idempotent `state/pattern-context.md`. Contract test `tests/daily-startup.test.cjs`. Second build from the autonomous-workflows design (W1)
...
### Security
- **Installer no longer destroys the native plugin store on uninstall** — `bin/install.js` `uninstallGovernance` previously `rmSync`'d the entire `plugins/` directory; for a global install that is `~/.claude/plugins`, i.e. Claude Code's own store (`repos/`, `config.json`, `cache/`, every marketplace plugin). It now removes only the GSD-owned plugin subdirs (enumerated from the package `plugins/`, falling back to the known constant list), leaving everything else intact.
- **Governance installs to the location Claude Code actually reads, and fails loud** — for global installs, governance `CLAUDE.md`, `context/`, and `settings.json` were written to the parent of `~/.claude` (`~/CLAUDE.md`, `~/settings.json`), so the governance hooks/permissions never loaded and the CLAUDE.md was never read. They now resolve under the config dir for global installs (`~/.claude/…`) while local/project installs keep `CLAUDE.md`/`context/` at the project root (the correct project-memory convention); `settings.json` always lands inside the config dir. A missing governance source is now recorded in the installer's failure summary instead of being silently skipped while reporting success.
- **`uat run-automated` is no longer a shell-injection sink** — `get-shit-done/bin/lib/uat-runner.cjs` ran `execSync` on shell strings built by interpolating PLAN.md `must_haves.truths` content, so a crafted must_have (a `` `…` `` / `$(…)` payload) executed arbitrary commands. Assertions now use an argv contract (`safeExec`/`spawnSync`, no shell) with PLAN.md values passed as data arguments; only the two static pipeline patterns (test suite, coverage) remain shell-executed and contain no interpolation.
- **Autonomous workflows no longer force-destroy work without consent** — `workflows/bug-to-branch.md` no longer auto-runs `git reset --hard` on test failure (it prints the rollback command for the operator instead), and `workflows/complete-milestone.md` no longer silently escalates `git branch -d` to `git branch -D`, which discarded unmerged commits the delete gate never authorized.
- **Corrupt `settings.json` is preserved, not silently overwritten** — `readSettings` swallowed JSON parse errors and returned `{}`, so a syntactically-broken user `settings.json` was treated as empty and then overwritten wholesale. It now writes a timestamped `.corrupt.<ts>` backup and warns before proceeding.

  **Migration note (governance settings location):** existing global installs placed governance files under `~/` (`~/settings.json`, `~/CLAUDE.md`, `~/context/`). This release writes them to the corrected `~/.claude/…` location; re-installing lands governance correctly. Old `~/settings.json` governance entries may be left behind — they were inert and can be removed manually.
