...
// ─────────────────────────────────────────────────────────────────────────────
// commit-to-subrepo command
// ─────────────────────────────────────────────────────────────────────────────

describe('commit-to-subrepo command', () => {
  const { createTempGitProject } = require('./helpers.cjs');
  const { execSync: execSyncCP } = require('child_process');
  let tmpDir;
  ...
  test('missing message — error', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ sub_repos: ['subrepo-a'] })
    );
    ...
    assert.ok(combined.includes('message'), 'should mention message required');
  });

  test('no sub_repos configured — error', () => {
    tmpDir = createTempGitProject();
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({})
    ...
    const result = runGsdTools(
      ['commit-to-subrepo', '--message', 'test', '--files', 'foo.txt'],
      tmpDir
    );
    assert.strictEqual(result.success, false, 'should fail without sub_repos');
    const combined = (result.output + ' ' + (result.error || '')).toLowerCase();
    assert.ok(combined.includes('sub_repos'), 'should mention sub_repos');
  });

  test('no files provided — error', () => {
    tmpDir = createTempGitProject();
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ sub_repos: ['subrepo-a'] })
    ...
    const result = runGsdTools(
      ['commit-to-subrepo', '--message', 'test'],
      tmpDir
    );
    assert.strictEqual(result.success, false, 'should fail without --files');
    const combined = (result.output + ' ' + (result.error || '')).toLowerCase();
    assert.ok(combined.includes('files'), 'should mention files required');
  });

  test('successful commit to sub-repo', () => {
    tmpDir = createTempGitProject();
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ sub_repos: ['subrepo-a'] })
    ...
    // Initialize subrepo-a as its own git repo
    const subrepoPath = path.join(tmpDir, 'subrepo-a');
    fs.mkdirSync(subrepoPath, { recursive: true });
    execSyncCP('git init', { cwd: subrepoPath, stdio: 'pipe' });
    execSyncCP('git config user.name "Test"', { cwd: subrepoPath, stdio: 'pipe' });
    execSyncCP('git config user.email "test@test.com"', { cwd: subrepoPath, stdio: 'pipe' });
    execSyncCP('git config commit.gpgsign false', { cwd: subrepoPath, stdio: 'pipe' });
    fs.writeFileSync(path.join(subrepoPath, 'README.md'), '# Subrepo A\n');
    execSyncCP('git add -A', { cwd: subrepoPath, stdio: 'pipe' });
    execSyncCP('git commit -m "initial"', { cwd: subrepoPath, stdio: 'pipe' });

    // Create a new file to commit via the command
    fs.writeFileSync(path.join(subrepoPath, 'newfile.txt'), 'hello\n');

    const result = runGsdTools(
      ['commit-to-subrepo', '--message', 'add file', '--files', 'subrepo-a/newfile.txt'],
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.repos['subrepo-a'].committed, true, 'subrepo-a should have committed');
    assert.ok(parsed.repos['subrepo-a'].hash, 'should have a commit hash');
  });

  test('unmatched files warning', () => {
    tmpDir = createTempGitProject();
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ sub_repos: ['subrepo-a'] })
    ...
    // Create files — one matched, one unmatched
    fs.writeFileSync(path.join(subrepoPath, 'file.txt'), 'matched\n');
    const otherDir = path.join(tmpDir, 'other');
    fs.mkdirSync(otherDir, { recursive: true });
    fs.writeFileSync(path.join(otherDir, 'file.txt'), 'unmatched\n');

    const result = runGsdTools(
      ['commit-to-subrepo', '--message', 'mixed files', '--files', 'subrepo-a/file.txt', 'other/file.txt'],
      tmpDir
    );
    // Command should succeed (matched file commits) but warn about unmatched
    assert.ok(result.success, `Command failed: ${result.error}`);
    const parsed = JSON.parse(result.output);
    assert.ok(
      (parsed.unmatched && parsed.unmatched.includes('other/file.txt')) ||
