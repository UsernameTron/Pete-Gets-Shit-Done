...
Vibecoding (or being ANYTHING but an OG Full Stack Dev) has a bad reputation. You describe what you want, AI generates code, and you get inconsistent garbage that falls apart at scale.
...
For Claude Code, the **governance layer installs by default** — CLAUDE.md, hooks, permissions, and context reference docs. Use `--no-governance` to skip it.
...
GSD includes a built-in governance framework for Claude Code (formerly [claude-code-kickstart](https://github.com/UsernameTron/claude-code-kickstart)) that establishes project standards, safety guardrails, and session workflows out of the box. Installed by default for the Claude Code runtime.

### What It Provides

**Global CLAUDE.md Template** (~360 lines)
A complete operating system for Claude Code sessions. Defines identity, session initialization sequence, development lifecycle, code standards, coverage requirements (90% overall / 80% per module / 95% security-critical), documentation standards, git workflow rules, and a do-not-touch list. References GSD commands as the execution engine.

**10 Safety Hooks**

| Event | Hook | What it does |
|-------|------|-------------|
| SessionStart | Project scanner | Auto-detects project state, git branch, existing tasks |
| PreToolUse | Branch protection | Blocks direct commits to main/master |
| PreToolUse | Staged file blocker | Prevents staging private files (state/, context/, .env) |
| PreToolUse | Docs checker | Requires CLAUDE.md, README.md, DEVOPS-HANDOFF.md before commit |
| PreToolUse | Secrets scanner | Detects API keys and credentials in staged files |
| PreToolUse | Nested repo detector | Warns about nested git repositories |
| PreToolUse | Pre-push dirty check | Blocks push with uncommitted changes |
| PostToolUse | File type advisor | Suggests better approaches for certain file operations |
| Stop | Uncommitted files checker | Warns about unsaved work when session ends |
| PreCompact | Task state preserver | Preserves task state before context compaction |

**Permission Rules**
Pre-configured allow/deny rules for common operations — safe defaults that prevent accidental damage while keeping workflows fast.

**6 Context Reference Docs**
On-demand reference loaded via `@context/` imports:

| Doc | Coverage |
|-----|----------|
| `cli-reference.md` | Claude Code CLI commands, flags, keyboard shortcuts, piping |
| `skill-creation-guide.md` | SKILL.md structure, frontmatter, progressive disclosure, anti-patterns |
| `mcp-setup-guide.md` | MCP transport types, scopes, authentication, .mcp.json format |
| `subagent-guide.md` | Agent frontmatter, model selection, tool access, built-in agents |
| `hooks-guide.md` | Hook events, matchers, exit codes, JSON output, environment variables |
| `settings-reference.md` | Settings scopes, permissions, sandbox, environment variables |

**2 Plugin Engines**

| Plugin | What it does | Components |
|--------|-------------|------------|
| **claude-mcp-ecosystem** | Session management and agent lifecycle | 9 commands (/prime, /wrap, /agents, /agent-setup, /agent-status, /agent-diagnose, /agent-add, /agent-remove, /agent-reset), 7 skills, 6 subagent pipeline workers |
| **claude-code-factory** | Extension generation from natural language | 38 skills (generators, reference library, intelligence layer), 10 specialist subagents |

The MCP ecosystem plugin gives you session commands: `/prime` boots your session (loads context, checks state, detects continuity), `/wrap` closes it (logs work, records decisions, notes next steps), and the `/agent-*` commands manage persistent specialist agents that remember context across sessions.

The code factory plugin generates Claude Code extensions from plain English: "create a hook that blocks force-push", "write a skill for database migrations", "build a plugin for our team's review process." 38 skills covering generators, reference documentation, quality gates, and an intent engine that routes your request to the right builder.

**Project Scaffolding**
`--scaffold` creates the full project structure:
...
| Flag | Description |
|------|-------------|
| `--no-governance` | Skip governance layer (GSD execution engine only) |
| `--scaffold` | Create project directory structure |
| `--plugins` | Register governance plugins (claude-mcp-ecosystem, claude-code-factory) |

```bash
# Default: GSD execution engine + governance framework
npx get-shit-done-cc --claude --global

# Execution engine only, no governance
npx get-shit-done-cc --claude --global --no-governance

# Full setup: engine + governance + project scaffolding + plugins
npx get-shit-done-cc --claude --global --scaffold --plugins
```

### How the Layers Compose

The governance framework and execution engine are complementary:

- **GSD commands** handle the build pipeline: `/gsd:discuss-phase` → `/gsd:plan-phase` → `/gsd:execute-phase` → `/gsd:verify-work` → `/gsd:ship`
- **Governance hooks** enforce safety: block commits to main, scan for secrets, require documentation, protect private files
...
| `/gsd:forensics [desc]` | Post-mortem investigation of failed workflow runs |
...
| Command | What it does |
|---------|--------------|
| `/prime` | Boot session — load context, check state, detect continuity from last session |
| `/wrap` | Close session — log work, record decisions, note next steps |
| `/agents` | Quick overview of all specialist agents |
| `/agent-setup` | Analyze codebase and build an agent team automatically |
| `/agent-status` | Check health and status of deployed agents |
| `/agent-diagnose` | Find and fix what's broken with agents |
| `/agent-add` | Add a new specialist agent |
| `/agent-remove` | Remove a specialist agent |
| `/agent-reset` | Reset an agent's memory (keeps the agent, clears what it learned) |
...
| `/gsd:quick [--full] [--discuss] [--research]` | Execute ad-hoc task with GSD guarantees |
| `/gsd:health [--repair]` | Validate `.planning/` directory integrity, auto-repair with `--repair` |
| `/gsd:stats` | Display project statistics — phases, plans, requirements, git metrics |
| `/gsd:profile-user [--questionnaire] [--refresh]` | Generate developer behavioral profile for personalized responses |
...
├── commands/gsd/               # 57 GSD slash commands
├── agents/                     # 18 specialized agents
├── hooks/                      # 5 runtime hooks
├── get-shit-done/              # Core skill (templates, workflows, references)
├── governance/                 # Governance layer
│   ├── templates/
│   │   ├── global/             # CLAUDE.md, settings-hooks.json, settings-permissions.json
│   │   ├── context/            # 6 reference docs (CLI, skills, hooks, MCP, settings, subagents)
│   │   └── project/            # Per-project starters (README, DEVOPS-HANDOFF, lessons)
│   ├── scripts/                # scaffold-project.sh, health-check.sh, install-plugins.sh
│   └── tests/                  # 5 shell test suites (146 assertions)
├── plugins/
│   ├── claude-mcp-ecosystem/   # Session commands + agent lifecycle (9 cmds, 7 skills, 6 agents)
│   └── claude-code-factory/    # Extension generation (38 skills, 10 subagents)
└── tests/                      # 295 test suites (1,517 assertions)
```
...
**Governance hooks add an additional layer:**
- **Secrets scanner** — Detects API keys and credentials in staged files before commit
- **Branch protection** — Blocks direct commits to main/master
- **Private file protection** — Prevents staging sensitive files (state/, context/, .env)
...
**Governance health check failing?**
```bash
bash governance/scripts/health-check.sh
```
Validates CLAUDE.md, hooks, permissions, context docs, and environment variables. Reports which components are missing.
...
This removes all GSD commands, agents, hooks, governance layer, and settings while preserving your other configurations.
...
**Claude Code is powerful. GSD makes it reliable. Governance keeps it safe.**
