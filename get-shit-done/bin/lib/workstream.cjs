  ...
  if (isFlatMode && options.migrate !== false) {
    ...
    if (hasExistingWork) {
      ...
      if (migrateName) {
        ...
        } catch { /* intentional: milestone info unavailable — use default name */
...
function cmdWorkstreamList(cwd, raw) {
  ...
  for (const entry of entries) {
    ...
    for (const d of phaseDirs) {
      ...
      } catch { /* intentional: skip unreadable phase directories during progress count */ }
    }
    ...
    } catch { /* intentional: STATE.md may not exist for workstream status */ }
...
function cmdWorkstreamStatus(cwd, name, raw) {
  ...
  for (const dir of readSubdirectories(p.phases).sort()) {
    ...
    } catch { /* intentional: skip unreadable phase directories during detail listing */ }
  }
  ...
  } catch { /* intentional: STATE.md may not exist for workstream detail */ }
...
function cmdWorkstreamComplete(cwd, name, options, raw) {
  ...
  try { fs.rmdirSync(wsDir); } catch { /* intentional: workstream dir may have leftover files */ }

  let remainingWs = 0;
  try {
    remainingWs = fs.readdirSync(wsRoot, { withFileTypes: true }).filter(e => e.isDirectory()).length;
    if (remainingWs === 0) fs.rmdirSync(wsRoot);
  } catch { /* intentional: best-effort cleanup of empty workstreams root */ }
  ...
  for (const entry of entries) {
    ...
    for (const d of phaseDirsProgress) {
      try {
        ...
        if (plans.length > 0 && summaries.length >= plans.length) completedCount++;
      } catch { /* intentional: skip unreadable phase directories in dashboard */ }
    }
    ...
    } catch { /* intentional: ROADMAP.md may not exist for phase count */ }
    ...
    try {
      ...
      status = stateExtractField(stateContent, 'Status') || 'unknown';
      currentPhase = stateExtractField(stateContent, 'Current Phase');
    } catch { /* intentional: STATE.md may not exist for dashboard status */ }
...
function getOtherActiveWorkstreams(cwd, excludeWs) {
  ...
  for (const entry of entries) {
    ...
    } catch { /* intentional: STATE.md may not exist for next-action status */ }
    ...
    for (const d of phaseDirsOther) {
      try {
        ...
        const summaries = filterSummaryFiles(phaseFiles);
        if (plans.length > 0 && summaries.length >= plans.length) completedCount++;
      } catch { /* intentional: skip unreadable phase directories in next-action */ }
    }
