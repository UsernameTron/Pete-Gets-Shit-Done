...
function cmdScaffold(cwd, type, options, raw) {
  const validTypes = ['context', 'uat', 'verification', 'phase-dir'];
  if (!validTypes.includes(type)) {
    error(`Unknown scaffold type: ${type}. Available: ${validTypes.join(', ')}`);
  }

  const { phase, name } = options;

  if (!phase && type !== 'phase-dir') {
    error(`--phase required for scaffold ${type}`);
  }
