...
**If mode = "both":** Continue to `automated_uat` after schema check completes.
</step>
...
2. Parse the JSON results. Extract passed, failed, and manual counts.
...
**Read-only guarantee:** All commands generated by the pattern registry are read-only
(file existence checks, grep, diff, node -p). No file writes occur during automated verification.
</step>
