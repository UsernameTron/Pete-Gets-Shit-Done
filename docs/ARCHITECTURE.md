...
The governance layer provides session management, safety guardrails, and project standards enforcement for Claude Code. It is installed by default alongside the GSD execution engine and can be opted out with `--no-governance`.
...
governance/
├── templates/
│   ├── global/                    # User-wide templates
│   │   ├── CLAUDE.md              # Global configuration template (session commands, lifecycle, standards)
│   │   ├── settings-hooks.json    # 10 hook definitions (branch protection, secrets scan, docs check, etc.)
│   │   └── settings-permissions.json  # Permission allow/deny rules
│   ├── project/                   # Per-project templates
│   │   ├── CLAUDE.md              # Project CLAUDE.md template
│   │   ├── README.md              # Project README template
│   │   ├── DEVOPS-HANDOFF.md      # DevOps handoff template
│   │   └── lessons.md             # Lessons learned template
│   └── context/                   # Reference documents (6 guides)
│       ├── cli-reference.md       # Claude Code CLI quick reference
│       ├── skill-creation-guide.md # Skill authoring guide
│       ├── mcp-setup-guide.md     # MCP server configuration
│       ├── subagent-guide.md      # Subagent creation and management
│       ├── hooks-guide.md         # Hooks configuration reference
│       └── settings-reference.md  # Settings and permissions reference
├── scripts/
│   ├── scaffold-project.sh        # Create project directory structure
│   ├── install-plugins.sh         # Register official governance plugins
│   └── health-check.sh            # Validate governance installation
└── tests/
    ├── test_health_check.sh       # Health check validation
    ├── test_install.sh            # Installation tests
    ├── test_install_plugins.sh    # Plugin installation tests
    ├── test_integration.sh        # Integration tests
    └── test_scaffold.sh           # Scaffolding tests
```
...
```
plugins/
├── claude-mcp-ecosystem/          # Agent lifecycle management
│   └── Session commands (/prime, /wrap, /agents, /agent-setup)
│   └── 3-layer agent routing (project-guide → concierge/companion → workers)
│   └── Workspace governance
└── claude-code-factory/           # Extension generation
    └── 35 skills for building Claude Code extensions
    └── 10 specialist subagents
    └── Reference library (hooks, skills, settings, agents, plugins, MCP)
```

### Composition with GSD Execution Engine

The governance layer and GSD execution engine are complementary systems:

| Concern | GSD Execution Engine | Governance Layer |
|---------|---------------------|-----------------|
| Planning | `/gsd:discuss-phase`, `/gsd:plan-phase` | — |
| Execution | `/gsd:execute-phase` (wave parallelization) | — |
| Verification | `/gsd:verify-work` | — |
| Session management | `/gsd:pause-work`, `/gsd:resume-work` | `/prime`, `/wrap`, `/revise-claude-md` |
| Safety hooks | `gsd-context-monitor.js`, `gsd-prompt-guard.js` | Branch protection, secrets scanning, docs checking, private file protection |
| Agent management | GSD agents (researcher, planner, executor, verifier) | MCP ecosystem agents (architect, scaffolder, validator, auditor) |
| Project standards | `.planning/` artifacts | CLAUDE.md template, coverage standards, documentation standards |
| Plugin ecosystem | — | `claude-mcp-ecosystem`, `claude-code-factory` |

The governance layer's hooks run alongside GSD's hooks in the same `settings.json`. GSD commands operate within the guardrails that governance establishes (branch protection, docs requirements, secrets scanning). Both systems use `settings.json` for hook registration and share the same Claude Code runtime.
