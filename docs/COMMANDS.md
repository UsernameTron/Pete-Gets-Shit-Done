...
These commands are provided by the governance layer's plugin engines (claude-mcp-ecosystem and claude-code-factory). They manage session lifecycle, agent infrastructure, and project standards.

### `/prime`

Boot a session — load context, lessons, state, agents, and git status.

**Behavior:** Reads CLAUDE.md, tasks/lessons.md, .planning/STATE.md (or tasks/todo.md), context files, agent definitions, and git state. Reports initialization summary.

```bash
/prime                              # Start-of-session bootstrap
```

### `/wrap`

Close a session — log work, record decisions, note next steps.

**Behavior:** Updates .planning/STATE.md with session handoff, appends to session-log.md.
...
Quick single commit with conventional message.
...
Branch + commit + push + PR in one shot.

```bash
/commit-push-pr                     # Full ship workflow
```

### `/clean-gone`

Remove stale local branches that have been deleted on the remote.
...
```bash
/revise-claude-md                   # Update CLAUDE.md with session insights
```

### `/agents`

List deployed specialist agents.
...
Initial agent deployment for a project (Phase 0 bootstrap).

**Behavior:** Scans project, designs agent roster, creates agent files and memory, validates structure.

```bash
/agent-setup                        # Deploy agent infrastructure
```
...
Diagnose agent issues when something is broken.

```bash
/agent-diagnose                     # Investigate agent problems
```
