...
Source: ported from Subagent Lifecycle Suite v3.0.0 with a GSD tool-profile appendix.
...
## Appendix: Tool Profiles

Three standard tool profiles for GSD agents:

**Read-only agents** (Explorers, Guardians): `Read, Glob, Grep, Bash`
**File-creating agents** (Builders): `Read, Write, Bash, Glob, Grep`
**Code-modifying agents** (Surgeons): `Read, Write, Edit, Bash, Glob, Grep`

Prefer `disallowedTools` over explicit `tools` lists when the intent is "everything
except X" — this is future-proof as Claude Code adds new tools. Guardians in
particular should use `disallowedTools: Write, Edit` rather than enumerating the
read-only subset.

## GSD Archetype Tagging

When auditing or backfilling a GSD agent, tag it with one archetype first. The tag
determines the expected frontmatter shape, tool profile, and hygiene template. If an
agent doesn't fit exactly one archetype, it is probably doing two jobs and should be
split.
