...
should_skip_file() {
  ...
  # Fallback: skip files that `file --mime` reports as non-text
  # This catches binaries with unrecognized extensions or no extension
  if [[ -f "$file" ]]; then
    local mime
    mime=$(file --brief --mime-type "$file" 2>/dev/null || echo "text/plain")
    case "$mime" in
  ...
  case "$mode" in
    --diff)
      ...
      git diff --name-only --diff-filter=ACMR "$base"...HEAD 2>/dev/null \
        | grep -vE '\.(png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot|otf|zip|tar|gz|bz2|xz|pdf|mp3|mp4)$' || true
      ;;
    --file)
      if [[ -f "$1" ]]; then
        echo "$1"
    ...
    --dir)
      ...
      find "$dir" -type f ! -path '*/node_modules/*' ! -path '*/.git/*' ! -path '*/dist/*' \
        ...
        ! -name '*.zip' ! -name '*.tar' ! -name '*.gz' ! -name '*.pdf' \
        ! -name '*.mp3' ! -name '*.mp4' ! -name '.DS_Store' 2>/dev/null || true
      ;;
    --stdin)
      cat
      ;;
...
extract_and_check_blobs() {
  ...
  # Fast pre-check: skip files with no base64-like blobs at all
  ...
  while IFS= read -r line; do
    ...
    while IFS= read -r blob; do
      ...
      # Count printable ASCII characters (LC_ALL=C avoids 'Illegal byte sequence' on macOS)
